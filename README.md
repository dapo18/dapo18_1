...
Стратегия «разделяй и властвуй» заключается в том, что сложная задача разбивается на несколько меньших подзадач того же типа, которые решаются рекурсивно, а затем их результаты объединяются в общее решение.

Пример — сортировка слиянием:
 1. Массив делится на две половины.
 2. Каждая половина сортируется рекурсивно тем же методом.
 3. Два отсортированных подмассива объединяются в один, сравнивая элементы поочерёдно.

Результат: на выходе — полностью отсортированный массив.
Временная сложность: O(n log n), так как выполняется log n уровней деления и на каждом уровне требуется O(n) операций слияния.
